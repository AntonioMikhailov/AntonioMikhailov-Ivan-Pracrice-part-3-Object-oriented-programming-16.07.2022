...
window.addEventListener('DOMContentLoaded', function () {
  ...
  videoPlayer.init(); //video в Модулях

  new _modules_playVideo__WEBPACK_IMPORTED_MODULE_2__["default"]('.module__video .play', '.overlay').init(); //передаем в параметры новый экземпляр класса -  объект из селекторов
  ...
  var showUpSlider = new _modules_slider_slider_mini__WEBPACK_IMPORTED_MODULE_1__["default"]({
    ...
    // точку не ставим перед классом т.к. будет add-remove class использоваться
    animate: true
  });
  showUpSlider.init(); // остальные слайдеры инициализируем - на 3 стр
  ...
  modulesSlider.init(); //слайдер на 5 стр

  var feedSlider = new _modules_slider_slider_mini__WEBPACK_IMPORTED_MODULE_1__["default"]({
    container: '.feed__slider',
    prev: '.feed__slider .slick-prev',
    // ограничим секцией селекторы
    next: '.feed__slider .slick-next',
    activeClass: 'feed__item-active' // точку не ставим перед классом т.к. будет add-remove class использоваться

  });
  feedSlider.init(); // модуль Difference

  new _modules_difference__WEBPACK_IMPORTED_MODULE_3__["default"]('.officerold', '.officernew', '.officer__card-item').init(); // можно так вызвать
  //  const Diff = new Difference('.officerold', '.officernew', '.officer__card-item' );
  // Diff.init();
  //Формы - можно по тегу или по классу

  new _modules_form__WEBPACK_IMPORTED_MODULE_4__["default"]('.form').init(); //Аккордеон Info

  new _modules_showinfo__WEBPACK_IMPORTED_MODULE_5__["default"]('.plus__content').init(); //скачивание файла
...
function () {
  ...
  function Difference(oldOffiser, newOfficer, items) {
    _classCallCheck(this, Difference);

    try {
      this.oldOffiser = document.querySelector(oldOffiser);
      this.newOfficer = document.querySelector(newOfficer); //для карточек делаем отдельную переменную для каждого блока

      this.oldItems = this.oldOffiser.querySelectorAll(items);
      this.newItems = this.newOfficer.querySelectorAll(items); //т.к. карточки будем получать из двух блоков то селекторы будем передавать отдельно

      this.items = items; //Добавляем счетчик для карточек, чтобы следить когда нужно будет скрыть последнюю карточку с Плюсом
  ...
  _createClass(Difference, [{
    key: "bindTriggers",
    value: function bindTriggers(container, itemsCards, counter) {
      ...
      container.querySelector('.plus').addEventListener('click', function () {
        ...
        if (counter !== itemsCards.length - 2) // -2 т.к. еще с Плюсом
          ...
          // когда дошли до последней - показываем ее и удалем самую последнюю с Плюсом 17-11
          itemsCards[counter].style.display = 'flex';
    ...
    value: function init() {
      try {
        ...
        this.hideItems(this.oldItems); //затем для второго

        this.hideItems(this.newItems); //также и для кнопок Плюс - два раза вызываем с разными селекторами
...
function () {
  function Download(triggers) {
    ...
    this.btns = document.querySelectorAll(triggers); //теперь нам нужны путь по которому будет браться файл -  в реале разные пути - решить можно через switch-case - определяеп на какую кликнули и подставляем нужный путь для этой кнопки
  ...
  _createClass(Download, [{
    ...
    value: function downloadItem(path) {
      ...
      element.setAttribute('href', path); // и также атрибут download нужен

      element.setAttribute('download', 'nice-picture'); //это элемент файл не появится на странице и не должен
      ...
      element.click(); // после клика сразу удаляем элемент
...
function () {
  function Form(forms) {
    ...
    this.inputs = document.querySelectorAll('input'); //для проверки поле почты на кириллицу - можно по атрибуту [type = "email"]

    this.emailInput = document.querySelectorAll('input[name="email"]'); //оповещаем юзера о процессе отправки - создадим объект с текстами
  ...
  _createClass(Form, [{
    ...
    value: function checkEmail() {
      ...
      this.emailInput.forEach(function (item) {
    ...
    value: function initMask() {
      var setCursorPosition = function setCursorPosition(pos, elem) {
        ...
        elem.focus(); // далее нужен метод setSelectionRange - но он не поддерживается старыми бр. и нам надо написать полифил

        if (elem.setSelectionRange) {
          elem.setSelectionRange(pos, pos); // установим курсор
        } else if (elem.createTextRange) {
          // если не поддерживает то метод range
      ...
      function createMask(event) {
        ...
        var matrix = '+1 (___) ___-___'; //итератор

        var i = 0; //получаем все Нецифры  и заменяем пустым - на основе матрицы

        var def = matrix.replace(/\D/g, ''); // все Нецифры чистим  - на основе ввода юзера

        var val = this.value.replace(/\D/g, ''); //сверяем кол. во симвлов. Если юзер начнет удалаять из матрицы +7 то он не сможет это сделать
          ...
          switch (_context.prev = _context.next) {
            case 0:
              ...
              return regeneratorRuntime.awrap(fetch(url, {
                // ждать должен
                method: "POST",
                // вот где ошибка была!!! надо "POST"  а не POST
                body: data //для formData заголовок не надо добавлять
    ...
    value: function init() {
      ...
      this.forms.forEach(function (item) {
        item.addEventListener('submit', function (e) {
          ...
          var statusMessage = document.createElement('div'); //добавми стили

          statusMessage.style.cssText = "\n      margin-top: 15px;\n      font-size: 18px;\n      color: white;\n      ";
          item.parentNode.appendChild(statusMessage); //родитель формы
          //вставляем контент

          statusMessage.textContent = _this.message.loading; //Идет загрузка
          //формируем данные для формы

          var formData = new FormData(item); // передали формы все две
          //вызываем ф. и передаем путь api и данные формы

          _this.postData(_this.path, formData) // т.к. возвращается промис - обрабатываем через then
          .then(function (res) {
...
function () {
  ...
  function VideoPlayer(triggers, overlay) {
    ...
    // close кнопку передавать не будем т.к. они есть во всех модалках а будем здесь инициализировать
    this.btns = document.querySelectorAll(triggers);
    this.overlay = document.querySelector(overlay);
    this.close = this.overlay.querySelector('.close'); //привязываем изменения контекста вызова к this. 21-14
  ...
  _createClass(VideoPlayer, [{
    ...
    value: function bindTriggers() {
      ...
      this.btns.forEach(function (item, i) {
        ...
        try {
          ...
          var blockedElem = item.closest('.module__video-item').nextElementSibling; //применяем остаток от деления
        ...
        item.addEventListener('click', function () {
          if (!item.closest('.module__video-item') || item.closest('.module__video-item').getAttribute('data-disabled') !== 'true') {
            ...
            _this.activeBtn = item; // по клику запускаем метод createPlayer
            ...
            if (document.querySelector('iframe#frame')) {
              // если уже есть, значит вызывали
              //просто показываем модалку и не создаем новый, чтобы позже по клику на close прятать именно его
              _this.overlay.style.display = 'flex'; //для кликов если кнопкок уже много 05-50

              if (_this.path !== item.getAttribute('data-url')) {
                // если не равен текущему атрибуту
                _this.path = item.getAttribute('data-url'); // то перезаписываем свойство
                // и запускаем видео функцией с API Utube

                _this.player.loadVideoById({
                  videoId: _this.path
                }); // загружаем новое видео по id и передаем объект

              }
            } else {
              // если нет еще
              _this.path = item.getAttribute('data-url'); //запускаем метод
    ...
    value: function bindClose() {
      ...
      this.close.addEventListener('click', function () {
        ...
        _this2.player.stopVideo(); // останавливаем при закрытии
    ...
    value: function createPlayer(url) {
      ...
      this.player = new YT.Player('frame', {
        // переменная YT пока нет - она будет подгружаться с U tube. Далее 'player' - это блок которй мы мы создадим в HTML  см. п1  документации
        height: '100%',
        // ставим максимум  - будет ограничен родителем
        width: '100%',
        videoId: "".concat(url),
        // важно! вместо кода M7lc1UVf-VE будем брать передавать код через параметр url - код подгружается с U tube
        events: {
          // но контекст this бдует теряться т.к. мы будем запускать вновом экземпляре класса - поэтому надо привязать через bind см. выше
    ...
    value: function onPlayerStateChange(state) {
      try {
        ...
        var blockedElem = this.activeBtn.closest('.module__video-item').nextElementSibling; //находим иконку Play с первого Плеера стобы позже применить ее к кнопке второго Плеера - через clone true нужен для глубокого копрования если передаем без параметров, иначе скопирует без содержимого только оболочку 14-20

        var playBtn = this.activeBtn.querySelector('svg').cloneNode(true); //когда именно мы будем выплнять - проверяем состояние 0 – воспроизведение видео завершено

        if (state.data == 0) {
          //но надо сначала проверить что он есть т.к. юзер может заново кликать
          if (blockedElem.querySelector('.play__circle').classList.contains('closed')) {
            blockedElem.querySelector('.play__circle').classList.remove('closed'); //удаляем иконку замка и заменяем на склонированную иконку playBtn

            blockedElem.querySelector('svg').remove();
            blockedElem.querySelector('.play__circle').appendChild(playBtn); //также надо заменить надпись на заголовка на Пелеере на Play Video 

            blockedElem.querySelector('.play__text').textContent = 'Play Video'; // убираем модификатор чтобы стиль текста поменять

            blockedElem.querySelector('.play__text').classList.remove('attention'); //также убрать свойство filter и opacity

            blockedElem.style.opacity = 1;
            blockedElem.style.filter = 'none'; //разблокируем второй Плеер
    ...
    value: function init() {
      if (this.btns.length > 0) {
        ...
        var tag = document.createElement('script'); // создаем тег

        tag.src = "https://www.youtube.com/iframe_api"; // путь

        var firstScriptTag = document.getElementsByTagName('script')[0]; //  скрипт должен быть самым первым в HTML
  ...
  _createClass(ShowInfo, [{
    ...
    value: function init() {
      this.btns.forEach(function (item) {
        item.addEventListener('click', function () {
          try {
            var sibling = item.closest('.module__info-show').nextElementSibling;
...
function (_Slider) {
  ...
  function MainSlider(page, btns) {
    ...
    return _possibleConstructorReturn(this, _getPrototypeOf(MainSlider).call(this, page, btns)); // взяли свойства  прототипа Slider
  ...
  _createClass(MainSlider, [{
      ...
      try {
        ...
        if (n == 3) {
          setTimeout(function () {
            ...
            _this.hansonImage.classList.add('animated'); // просто зазадет время


            _this.hansonImage.style.opacity = '1';

            _this.hansonImage.classList.add('slideInUp'); //  а этот стиль уже анимирует

          }, 3000);
        } else {
          // когда юзер уйдет со страницы прячем снова
          this.hansonImage.classList.remove('slideInUp');
        }
      } catch (error) {} //скрываем все слайды при первом показе
    ...
    value: function bindTriggers() {
      ...
      this.btns.forEach(function (item) {
        item.addEventListener('click', function () {
          _this2.plusSlides(1); // вызовет plusSlides  а тот метод showSlides
        ...
        item.parentNode.previousElementSibling.addEventListener('click', function (e) {
          ...
          _this2.slideIndex = 1; // меняем индекс на 1
    ...
    value: function render() {
      if (this.container) {
        // если на странице есть такой элемент то выполняется код
        try {
          this.hansonImage = document.querySelector('.hanson');
        } catch (error) {} // запускаем сразу показ первого слайда при загрузке страницы и передаем index


        this.showSlides(this.slideIndex); // Важно! у каждого экземпляра объекта будут свои индексы и салайдеры которые не будут пересекаться
...
function (_Slider) {
  ...
  function MiniSlider(container, next, prev, activeClass, animate, autoplay) {
    ...
    return _possibleConstructorReturn(this, _getPrototypeOf(MiniSlider).call(this, container, next, prev, activeClass, animate, autoplay)); // берем из прототипа
  ...
  _createClass(MiniSlider, [{
    ...
    value: function decorizeSlides() {
      ...
      this.slides.forEach(function (item) {
        item.classList.remove(_this.activeClass); //очищаем все анимации = прозрачности

        if (_this.animate == true) {
          // если было передано из инициализации
          //для заголовка слайда
          item.querySelector('.card__title').style.opacity = "0.4"; //для стрелки на слайде

          item.querySelector('.card__controls-arrow').style.opacity = "0";
        }
      });
      this.slides[0].classList.add(this.activeClass); // создаем анимацию для активных слайдов

      if (this.animate == true) {
        // если было передано из инициализации
        //для заголовка слайда
        this.slides[0].querySelector('.card__title').style.opacity = "1"; //для стрелки на слайде
    ...
    value: function bindTriggers() {
      ...
      }); // вызываем метод переключения вперед 

      this.prev.addEventListener('click', function () {
        console.log(22);
        var active = _this2.slides[_this2.slides.length - 1]; // последний слайд помещаем перед первым

        _this2.container.insertBefore(active, _this2.slides[0]);

        _this2.decorizeSlides(); // активируем анимацию также
...
var Slider = //передаем объект в виде деструктуризации - page, btns, next -  по умолчанию свойство - они могут быть а могут и нет - тогда пустая строка. ={} - если передадут без параметров сработает пустой объект
// будем для извлечение свойств использовать деструктуризацию. Порядок свойств в объекте не важен
function Slider() {
